...
import { HiPencil, HiTrash, HiEye, HiShoppingBag, HiFolder, HiTrendingUp, HiCheckCircle, HiXCircle } from 'react-icons/hi';
...
const AdminEquipment = ({
  ...
  const handleEquipmentSubmit = async (e) => {
    ...
    try {
      const equipmentData = {
        ...
        enabled: equipmentForm.enabled !== undefined ? equipmentForm.enabled : true
      };
      ...
      setEquipmentForm({ name: '', category: '', price: '', description: '', image: '', enabled: true });
      onRefresh();
    } catch (error) {
  ...
  const handleToggleEnabled = async (item) => {
    try {
      ...
      await updateDoc(doc(db, 'sportsItems', item.id), { enabled: newEnabledStatus });
      onRefresh();
    } catch (error) {
      console.error('Error toggling equipment enabled status:', error);
      Swal.fire({
        icon: 'error',
        title: 'Update Failed',
        text: 'Error updating equipment status. Please try again.',
        confirmButtonColor: '#10b981'
      });
    }
  };
  ...
  const paginatedItems = getPaginatedData(filteredItems);
  ...
  return (
    <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setCurrentPage(prev => ({ ...prev, equipment: 1 }));
              }}
          ...
          <div>
            ...
            >
              ...
              {[...new Set(sportsItems.map(item => item.category).filter(Boolean))].sort().map(category => (
      ...
      <PaginationControls data={filteredItems} />
            ...
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
              ...
              <button
                onClick={() => {
                  setShowEquipmentForm(false);
                  setEditingEquipment(null);
                  setEquipmentForm({ name: '', category: '', price: '', description: '', image: '', complexId: '' });
                }}
