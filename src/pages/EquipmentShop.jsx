...
const EquipmentShop = () => {
  ...
  const categories = ['all', ...new Set(sportsItems.map(item => item.category).filter(Boolean))];

  const filteredItems = filter === 'all' 
    ? sportsItems 
    : sportsItems.filter(item => item.category === filter);
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      const purchaseRef = await addDoc(collection(db, 'equipmentPurchases'), purchaseData);
      ...
      try {
        ...
        if (userEmail) {
          const { sendEquipmentEmail } = await import('../utils/emailService');
      ...
      await Swal.fire({
        ...
        text: 'Your equipment order has been placed.',
        confirmButtonColor: '#10b981'
      });
      ...
      <div className="flex-1 container mx-auto px-4 py-12">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            <h1 className="text-4xl font-bold text-gray-800 mb-4">Sports Equipment Shop</h1>
            <p className="text-gray-600 text-lg">
              Browse our wide selection of sports equipment and accessories
            </p>
          </div>
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
              <h3 className="text-2xl font-bold text-gray-800">Payment Preview</h3>
              <button
